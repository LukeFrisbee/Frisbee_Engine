...
namespace fengine {
	Shader::Shader(Device& device, const VkRenderPass& renderPass, const Shader::Settings& settings, const std::string& vertexPath, const std::string& fragPath)
		: m_device{ device }
	{
		_buildDescriptorSet();
	...
	void Shader::_buildDescriptorSet()
	{
		for (int frameIndex = 0; frameIndex < SwapChain::MAX_FRAMES_IN_FLIGHT; frameIndex++) {
			...
			bufferCreateInfo.size = sizeof(uboGlobal);
			...
			vmaCreateBuffer(m_device.vmaAllocator(), &bufferCreateInfo, &allocCreateInfo,
				&m_uniforms[0].buffers[frameIndex],
				&m_uniforms[0].allocations[frameIndex],
