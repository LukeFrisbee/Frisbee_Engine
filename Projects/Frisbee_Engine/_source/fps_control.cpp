...
namespace fengine {
	void FPSControl::moveInPlaneXZ(const FWindow& fWindow, float dt, RenderObject& gameObject)
	{
	...
	glm::vec3 FPSControl::getMouseWorldPosition(const FWindow& fWindow, const glm::mat4& viewMatrix, const glm::mat4& projectionMatrix) {
		...
		float x = ((2.0f * xPos) / extent.width) - 1.0f;
		float y = ((2.0f * yPos) / extent.height) - 1.0f;
